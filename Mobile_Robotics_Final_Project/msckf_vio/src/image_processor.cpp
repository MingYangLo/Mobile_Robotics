...
bool ImageProcessor::initialize() {
  ...
  detector_ptr= ORB::create(2000,1.2f,8,31,0,2,ORB::HARRIS_SCORE,31,10);
...
void ImageProcessor::ORBMatch(
  ...
  if(cam1_points.size() == 0) {
    stereoMatch(cam0_points, cam1_points, inlier_markers);return;}

  vector<DMatch> matches;
  matcher_ptr->match(descriptors0, descriptors1, matches);
  ...
  for (int i = 0; i < descriptors0.rows; i++) {
    if (matches[i].distance <= max(2 * min_dist, 30.0)) {
      inlier_markers.insert(inlier_markers.end(),1);
    }else{
      inlier_markers.insert(inlier_markers.end(),0);
    }
  }
