...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    img_out = np.zeros((h, w, 1)).astype(np.uint8)
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        masks = masks[:num_dets_to_consider, :, :, None]
        for i in range(num_dets_to_consider):
            temp_class_check = cfg.dataset.class_names[classes[i]]
            if (temp_class_check == 'car' or 
                temp_class_check == 'person' or 
                temp_class_check == 'motorcycle' or
                temp_class_check == 'bus' or
                temp_class_check == 'truck'):
                msk = masks[i, :, :, None]
                mask = msk.view(1, masks.shape[1], masks.shape[2], masks.shape[3])
                img_gpu = (mask.sum(dim=0) >= 1).float().contiguous() * 255
                img_numpy = img_gpu.byte().cpu().numpy().astype(int)
                img_out = np.bitwise_or(img_numpy, img_out)
    return img_out
...
def evalimage_cv_img(net:Yolact, img):
...
def img_callback(data):
    ...
    img_numpy = evalimage_cv_img(Yolactnet, cv_image)
    # cv2.imshow('test', img_numpy.astype(np.uint8))
    # cv2.waitKey(1)
    mask_pub.publish(bridge.cv2_to_imgmsg(img_numpy.astype(np.uint8), 'mono8'))
    print('process image')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        global Yolactnet
        Yolactnet = Yolact()
        Yolactnet.load_weights(args.trained_model)
        Yolactnet.eval()
        print(' Done.')

        global Yolactnet_cuda
        if args.cuda:
            Yolactnet_cuda = Yolactnet.cuda()

        global bridge
        bridge = CvBridge()
        rospy.init_node('img_segmentation', anonymous=False)
        global mask_pub
        mask_pub = rospy.Publisher('image_mask', Image, queue_size=10)
        rospy.Subscriber('/stereo/left/image_raw', Image, img_callback)
        rospy.spin()
        
        #evaluate(Yolactnet_cuda, dataset)
